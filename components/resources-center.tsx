    ...
    <div className="page-stack">
      ...
      <article className="panel configuration-panel">
        ...
        <div className="configuration-body">
          ...
          {tab === "certificates" ? <ResourceSection title="Certificados y evidencia" copy="Adjunta PDF, fotografías, proveedor, alcance, incertidumbre y fecha de vigencia." action="Adjuntar certificado" actionTutorialId="equipment-certificates" onAction={() => setModal("certificate")}><SimpleTable columns={[{ key: "code", label: "Código" }, { key: "equipment", label: "Equipo" }, { key: "type", label: "Tipo" }, { key: "provider", label: "Proveedor" }, { key: "issued", label: "Emitido" }, { key: "expires", label: "Vence" }, { key: "status", label: "Estado" }]} rows={certificates} /></ResourceSection> : null}
...
function ResourceSection({ title, copy, action, onAction, actionTutorialId, children }: Readonly<{ title: string; copy: string; action: string; onAction: () => void; actionTutorialId?: string; children: React.ReactNode }>) {
  return <section><div className="section-heading"><div><h2>{title}</h2><p>{copy}</p></div><button className="secondary-button" data-tutorial={actionTutorialId} onClick={onAction}><Plus size={15} /> {action}</button></div>{children}</section>;
}
